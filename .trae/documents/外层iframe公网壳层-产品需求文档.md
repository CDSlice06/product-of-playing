## 1. 产品概述
为现有 `02-project` 游戏新增一个纯净外层网页框架，使用 `iframe` 完整承载现有本地游戏，在不改动游戏玩法、界面、样式与功能的前提下，解决手机公网访问适配、加载稳定性、标题图标、自定义部署和访问一致性问题。
- 目标用户为需要将现有本地可玩游戏快速发布到公网、并希望朋友通过手机浏览器或微信等移动端入口直接访问游玩的项目维护者。
- 产品价值在于通过“壳层页面 + 内层游戏”的双层结构，把原有游戏稳定封装成可直接部署到 Vercel 的标准静态站点，同时降低移动端加载失败、路径异常、访问不一致的问题。
...
| 项目维护者 | 本地开发或部署 Vercel | 配置标题图标、部署参数、相对路径与外层适配策略 |
...
4. **部署配置层**：Vercel 静态部署配置、路由回退、资源路径修正、去除 Trae 相关标识。
...
| 部署配置 | 静态站点规则 | 为 Vercel 配置 SPA 回退、缓存策略、统一公开路径与图标资源 |

## 3. 核心流程
维护者在本地构建项目后，外层壳页作为唯一公开入口被部署到 Vercel。用户打开公网链接后，壳页先完成移动端视口初始化、首屏 loading、路径修正和网络兼容处理，再用相对路径加载内层游戏 `iframe`。当 `iframe` 成功载入后，壳页隐藏加载层并将交互交给内层游戏；若加载失败，则展示重试与回退提示，但不触碰内层游戏业务代码。
...
- 图标建议：使用纯净 favicon 与简洁品牌元素，不保留任何 Trae 标识或默认模板信息
